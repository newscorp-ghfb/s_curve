...
# Simulation  --------------------------------------------------------------------------------------
deltPrice <- Delt(tblPrice$adjusted, k=1:200)                                     # Quantmod Price k-period % difference
...
# Top / Bottom n Values  ------------------------------------------------------------------------------------
mtxDeltPriceBotN <- head(sort(mtxDeltPrice), 50)
mtxDeltPriceTopN <- head(sort(mtxDeltPrice,decreasing = TRUE), 50) 
mtxDeltPriceTopIdx <- which(mtxDeltPrice>=sort(mtxDeltPrice, decreasing = T)[50], 
  arr.ind = T)
mtxDeltPriceBotIdx <- which(mtxDeltPrice<=sort(mtxDeltPrice, decreasing = F)[50], 
  arr.ind = T)



bottom05 <- head(sort(mtxDeltPrice), 5)
top05 <- head(sort(mtxDeltPrice,decreasing = TRUE), 15) 
x <- which(mtxDeltPrice>=sort(mtxDeltPrice, decreasing = T)[5], arr.ind = T)
x.order <- order(mtxDeltPrice[x], decreasing = T)
y <- which.min(mtxDeltPrice>=sort(mtxDeltPrice, decreasing = T)[5], arr.ind = T)
which.m




top05 <- mtxDeltPrice[order(mtxDeltPrice)[1:5]]
# Create Matrix  -----------------------------------------------------------------------------------
mtxDeltPrice <- data.matrix(Delt(tblPrice$adjusted, k=1:200))
mtxDeltPrice <- data.matrix(deltPrice)
mtxDeltPrice[is.infinite(mtxDeltPrice) | is.na(mtxDeltPrice)] <- 0
rownames(mtxDeltPrice) <- rownames(mtxDeltPrice, do.NULL = FALSE, prefix = "")                      # Add Row Name - Index Value 
colnames(mtxDeltPrice) <- c(1:200)                                                                  # Add Column Name Value 1 - 200
# Top 2 Values  ------------------------------------------------------------------------------------
mm <- apply(mtxDeltPrice,2,function(xx)tail(sort(xx),10))
mmm <- apply(mtxDeltPrice,2,function(xx)tail(names(sort(xx)),10))
mtxDeltPriceTop50 <- order(mtxDeltPrice , na.last=TRUE, decreasing=TRUE)[1:10] 


m <- data.matrix(Delt(tblPrice$close, k=1:200)) 
m <- data.matrix(deltPrice, k=1:1)                                                                  # Create Price Matrix
...
colnames(m) <- c(1:1)                                                                               # Add Column Name Value 1 - 200